...
// Ilfov metro centres live in the București view, not the national map.
...
export function LocationsMap() {
  ...
  // when already visible), so clicking București doesn't jump the page.
  useEffect(() => {
    if (!mounted.current) {
    ...
    >
      ...
      >
        <img
          ...
          alt="Harta județelor României cu centrele Affidea"
          ...
          .map((p) => {
            ...
            return (
              <button
                ...
                aria-label={
                  p.sectors
                    ? "București — vezi centrele pe sectoare"
                    : `Centre Affidea în ${prettyCity(p.city)} (${p.count})`
                }
      ...
      >
        ...
        >
          <span aria-hidden>←</span> Înapoi la harta țării
        </button>

        <div className="relative w-full">
          <img
            src="/bucuresti-sectors.svg?v=5"
            alt="Municipiul București împărțit pe cele 6 sectoare"
            className="block h-auto w-full select-none"
            draggable={false}
          ...
          {BUC_ILFOV.map((i) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => openIlfov(i)}
                aria-label={`Centre Affidea în ${i.label} (Ilfov)`}
                className="group absolute z-20 grid h-7 w-7 -translate-x-1/2 -translate-y-1/2 place-items-center"
        ...
        <p className="mt-4 flex items-center justify-center gap-2 text-center text-xs text-muted">
          ...
          />
          Otopeni și Ștefănești sunt în județul Ilfov, lângă București
        </p>
      </div>
    </div>
  );
...
function openStatus(hours: DayHours[]): { open: boolean; label: string } {
  ...
  if (today && cur >= mins(today[0]) && cur < mins(today[1])) {
    return { open: true, label: `Deschis · se închide la ${today[1]}` };
  }
  for (let k = 0; k < 7; k++) {
    const d = (day + k) % 7;
    const h = hours[d];
    ...
    if (k === 0) {
      if (cur < mins(h[0]))
        return { open: false, label: `Închis · deschide azi la ${h[0]}` };
      continue;
    }
    const when = k === 1 ? "mâine" : DAYS[d];
    return { open: false, label: `Închis · deschide ${when} la ${h[0]}` };
  }
  return { open: false, label: "Închis" };
}
        ...
        <div className="flex items-start justify-between gap-3 border-b border-line px-4 pb-2.5 pt-3">
          <div>
            ...
            <p className="mt-0.5 text-[0.7rem] text-muted">
              {clinics.length
                ? `${clinics.length} ${clinics.length === 1 ? "centru" : "centre"} de imagistică`
                : "Fără centru de imagistică aici"}
            </p>
          </div>
          <button
            type="button"
